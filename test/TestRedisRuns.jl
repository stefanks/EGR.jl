...
myREfunction(problem, opts, sd, expIndices) = returnIfExists(client, problem, opts, sd, expIndices; outputLevel = 1)
...
myOpts= Opts(zeros(numVars); stepSizeFunction=constStepSize, stepSizePower=0, maxG=20, outputLevel=algOutputLevel)
...
myOpts= Opts(zeros(numVars); stepSizeFunction=constStepSize, stepSizePower=0, maxG=10, outputLevel=algOutputLevel)
alg(thisProblem(Task(() -> getSequential(numTrainingPoints, gradientOracle, restoreGradient)))	, myOpts,thisSD, myOutputOpts, myWriteFunction, myREfunction)
println(" gnum:     "*string(int(lrange(client, longKey*":gnum",0, -1))))
println(" origWant: "*string(int(lrange(client, longKey*":origWant",0, -1))))
...
myOpts= Opts(zeros(numVars); stepSizeFunction=constStepSize, stepSizePower=0, maxG=30, outputLevel=algOutputLevel)
alg(thisProblem(Task(() -> getSequential(numTrainingPoints, gradientOracle, restoreGradient)))	, myOpts, thisSD, myOutputOpts, myWriteFunction, myREfunction)
println(" gnum:     "*string(int(lrange(client, longKey*":gnum",0, -1))))
println(" origWant: "*string(int(lrange(client, longKey*":origWant",0, -1))))

myOpts= Opts(zeros(numVars); stepSizeFunction=constStepSize, stepSizePower=0, maxG=100000, outputLevel=algOutputLevel)
alg(thisProblem(Task(() -> getSequential(numTrainingPoints, gradientOracle, restoreGradient)))	, myOpts, thisSD, myOutputOpts, myWriteFunction, myREfunction)
println(" gnum:     "*string(int(lrange(client, longKey*":gnum",0, -1))))
println(" origWant: "*string(int(lrange(client, longKey*":origWant",0, -1))))

myOpts= Opts(zeros(numVars); stepSizeFunction=constStepSize, stepSizePower=0, maxG=100001, outputLevel=algOutputLevel)
