type EGRsd <: StepData
	...
	function EGRsd(s::Function, u::Function, beta::Function, numVars::Int64,dt::DataType, stepString::String, shortString::String)
		new(zeros(numVars),(Function,Function)[], dt[],0, s, u, beta, naturalEGRS, stepString, shortString)
	end
end

function EGRlin(c::Float64,r::Float64,numVars::Int64,ntp::Int64, beta::Function, newS::Bool, dt::DataType, stepString::String, shortString::String)
	EGRsd( (k,I)-> int(floor(k==0 ? 0 : c*(r/(r-1))^(k-1)))  >I ? I : int(floor(k==0 ? 0 : c*(r/(r-1))^(k-1))),  (k,I)-> int(floor(k==0 ? c*(r-1) : c*(r/(r-1))^(k-1))) > ntp - I ? ntp-I : int(floor(k==0 ? c*(r-1) : c*(r/(r-1))^(k-1))) , beta,  numVars, dt,	stepString, shortString)
end

function EGRquad(c::Float64,r::Float64,numVars::Int64,ntp::Int64, beta::Function, newS::Bool, dt::DataType, stepString::String, shortString::String)
	EGRsd( (k,I)-> int(floor(k==0 ? 0 : c*(r/(r-1))^(k-1)))  >I ? I : int(floor(k==0 ? 0 : c*(r/(r-1))^(k-1))),  (k,I)-> int(floor(k==0 ? c*(r-1) : c*(r/(r-1))^(k-1))) > ntp - I ? ntp-I : int(floor(k==0 ? c*(r-1) : c*(r/(r-1))^(k-1))) , beta,  numVars, dt,	stepString, shortString)
end
